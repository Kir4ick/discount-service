...
class SecondReservasionGroupMiddlewareTest extends TestCase
{
    public function testDifferenceSuccess(): void
    {
        $middleware = new SecondReservasionGroupMiddleware(0);
        ...
        $input = new CalculatePriceActionInput(
            $price,
            4,
            (new \DateTime())->setDate((int)$now->format('Y') + 1, 1, 9),
            (new \DateTime())->setDate((int)$now->format('Y'), 3, 9)
        );
